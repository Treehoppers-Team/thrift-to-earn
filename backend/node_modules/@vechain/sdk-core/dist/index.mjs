...
__export(core_exports, {
  ...
  vechain_sdk_core_ethers: () => ethers4
});
...
import { InvalidAbiDataToEncodeOrDecode as InvalidAbiDataToEncodeOrDecode2 } from "@vechain/sdk-errors";
...
import { InvalidAbiDataToEncodeOrDecode as InvalidAbiDataToEncodeOrDecode3 } from "@vechain/sdk-errors";
...
var Hex = class _Hex {
  ...
  get n() {
    ...
    throw new InvalidOperation2("Hex.n", "not an IEEE 754 float 64 number", {
      hex: this.toString()
    });
  }
  /**
...
var Keccak256 = class _Keccak256 extends HexUInt {
  ...
  static of(exp) {
    try {
      ...
      throw new InvalidOperation3("Keccak256.of", "hash error", {
        exp: `${exp}`,
        // Needed to serialize bigint values.
        e
      });
...
var Sha256 = class _Sha256 extends HexUInt {
  ...
  static of(exp) {
    try {
      ...
      throw new InvalidOperation4("Sha256.of", "hash error", {
        exp: `${exp}`,
        // Needed to serialize bigint values.
        e
      });
...
// src/vcdm/Address.ts
import {
  InvalidDataType as InvalidDataType4,
  ...
  static of(exp) {
    try {
      ...
      if (_Address.isValid(huint.toString())) {
        ...
        throw new InvalidDataType4("Address.of", "not a valid address", {
          huint
        });
      }
    } catch (error) {
...
import { InvalidOperation as InvalidOperation6 } from "@vechain/sdk-errors";
var Blake2b256 = class _Blake2b256 extends HexUInt {
  ...
  static of(exp) {
    try {
      ...
      throw new InvalidOperation6("Blake2b256.of", "hash error", {
        exp: `${exp}`,
        // Needed to serialize bigint values.
        e
      });
...
// src/vcdm/BloomFilter.ts
import { InvalidDataType as InvalidDataType5, InvalidOperation as InvalidOperation7 } from "@vechain/sdk-errors";
var BloomFilter = class _BloomFilter {
  ...
  join(other) {
    if (this.k === other.k) {
      ...
      throw new InvalidOperation7(
        "BloomFilter.join",
        "different length values",
        { this: this, other }
      );
    }
    throw new InvalidOperation7("BloomFilter.join", "different k values", {
      this: this,
      other
    });
  }
...
var FPN = class _FPN {
  ...
  compareTo(that) {
    if (this.isNaN() || that.isNaN())
      throw new InvalidOperation8("FPN.compareTo", "compare between NaN", {
        this: `${this}`,
        that: `${that}`
      });
...
var Revision = class _Revision extends Txt {
  ...
  static of(value) {
    try {
      ...
      throw new InvalidDataType8("Revision.of", "not a revision", {
        value: `${value}`
      });
    } catch (e) {
      throw new InvalidDataType8("Revision.of", "not a revision", {
...
// src/vcdm/ThorId.ts
import { InvalidDataType as InvalidDataType9 } from "@vechain/sdk-errors";
...
function bigNumberOf(value) {
  ...
  if (bn.isNaN())
    throw new InvalidDataType12("unitUtils.bigNumberOf()", "Not a number.", {
...
function digitsOfFractionalPart(value) {
  let d = 0;
  const i = value.abs().integerValue(BigNumber.ROUND_FLOOR);
...
function digitsOfIntegerPart(value) {
  let d = 0;
  let i = value.abs().integerValue(BigNumber.ROUND_FLOOR);
  while (i.gte(1)) {
    d++;
    i = i.div(10);
  }
...
import { InvalidDataType as InvalidDataType13 } from "@vechain/sdk-errors";
...
async function encrypt(privateKey, password) {
  ...
  const keystoreJsonString = await ethers3.encryptKeystoreJson(
    keystoreAccount,
    password,
    encryptOptions
  );
...
async function decrypt(keystore4, password) {
  ...
  try {
    return await ethers3.decryptKeystoreJson(
...
function isValid(keystore4) {
  return ethers3.isKeystoreJson(stringifyData(keystore4));
}
...
import {
  ...
  InvalidTransactionField as InvalidTransactionField2,
  NotDelegatedTransaction as NotDelegatedTransaction2
} from "@vechain/sdk-errors";
...
function signWithDelegator(transactionBody, signerPrivateKey, delegatorPrivateKey) {
  ...
  const transactionToSign = new Transaction(transactionBody);
  if (!transactionToSign.isDelegated)
    throw new NotDelegatedTransaction2(
      "signWithDelegator()",
...
import { InvalidTransactionField as InvalidTransactionField3 } from "@vechain/sdk-errors";
...
function _decodeReservedField(reserved) {
  if (reserved[reserved.length - 1].length === 0) {
    throw new InvalidTransactionField3(
...
export {
  ...
  unitsUtils,
  validateNumericKindData,
  ethers4 as vechain_sdk_core_ethers
};
